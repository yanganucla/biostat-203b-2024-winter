...
```{r}
#| eval: false
labevents.parquet <- arrow::write_dataset(labevents <- arrow::open_dataset("labevents.csv", format = "csv"), "~/mimic/hosp/labevents.parquet", format = "parquet")

system.time({
  labevents.parquet <- arrow::open_dataset("~/mimic/hosp/labevents.parquet", format = "parquet") %>%
    filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>%
    select(subject_id, itemid, charttime, valuenum) %>%
    collect()
})
...
labevents_parquet <- arrow::open_dataset("~/mimic/hosp/labevents.parquet", format = "parquet")
