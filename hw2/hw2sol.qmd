...
```
The Parquet file `labevents.parquet` is 1.1 GB. The ingest+select+filter process of the Parquet file takes 0.5 minutes. The number of rows in the result tibble is 24855908. The first 10 rows of the result tibble are:
...
cat("The number of rows in the result tibble is", nrow(labevents_parquet), ".")
