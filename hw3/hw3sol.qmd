...
sid_proc <- read_csv("~/mimic/hosp/procedures_icd.csv.gz",
  ...
  filter(subject_id == sid) |>
  left_join(read_csv("~/mimic/hosp/d_icd_procedures.csv.gz", show_col_types = FALSE), by = c("icd_code", "icd_version")
  ) |>
  print(width = Inf)
```
Diagnosis:
...
sid_diag <- read_csv("~/mimic/hosp/diagnoses_icd.csv.gz",
  ...
  filter(subject_id == sid) |>
  left_join(read_csv("~/mimic/hosp/d_icd_diagnoses.csv.gz", show_col_types = FALSE), by = c("icd_code", "icd_version")
  ) |>
  print(width = Inf)
```
...
ggplot(data = sid_1.2_final2) + 
  geom_point(mapping = aes(x = charttime, y = valuenum, color = factor(abbreviation))) + 
  geom_line(mapping = aes(x = charttime, y = valuenum, color = factor(abbreviation))) +
  facet_grid(vars(abbreviation), vars(stay_id), scales = "free_y") +
  theme_minimal() +
...
```{r}
labevents_tble1 <- arrow::open_dataset("labevents_pq", format = "parquet") %>%
