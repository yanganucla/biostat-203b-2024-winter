...
```{r}
sid_1.2_final <- chartevents_tble1 %>%
  filter(subject_id == sid) %>%
  print(width = Inf)
...
ggplot(data = sid_1.2_final) + 
  geom_point(mapping = aes(x = charttime, y = valuenum, color = label)) + geom_line(mapping = aes(x = charttime, y = valuenum, color = label)) +
  facet_grid(label ~ stay_id, scales = "free_y") +
