...
```
```{r}
sid_procedure$chartdate <- as.POSIXct(sid_procedure$chartdate)
...
ggplot() +
  ...
  geom_point(data = sid_procedure, aes(x = chartdate, y = "Procedure"), shape = 17, size = 3, color = "black") +
...
```{r}
sid_pat <- read_csv("~/mimic/icu/icustays.csv.gz") |>
  filter(subject_id == sid) |>
  print(width = Inf)
```
