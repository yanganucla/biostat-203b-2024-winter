...
ggplot() +
  ...
  labs(title = "Patient 10013310,F,70 years old",
       subtitle = "Cholera due to vibrio cholerae, Cholera due to vibrio cholerae el tor, Cholera, unspecified",
       x = "Calendar Time",
       y = "") +
...
```{r}
sid_dicd <- read_csv("~/mimic/hosp/d_icd_diagnoses.csv.gz") |>
  print(width = Inf)
```

```{r}
sid_proc <- read_csv("~/mimic/hosp/d_icd_procedures.csv.gz") |>
  print(width = Inf)
```
...
```{r}
ggplot(data = sid_adt) + 
  geom_point(mapping = aes(x = intime, y = careunit)) + 
  facet_grid(intime ~ careunit)
```
...
method1
```{r}
chartevents_tble2 <- chartevents_tble1 %>%
...
```{r}
chartevents_tble3 <- chartevents_tble2 %>%
...
```{r}
# chartevents_tble1 不left join ,Merge filtered_chartevents with icustays_tble
merged_data_chartevents <- merge(icustays_tble, chartevents_tble1, by = c("subject_id"), all.x = TRUE)

# Group by icustay_id and itemid, then filter for the first measurement within each ICU stay
chartevents_first <- merged_data_chartevents %>% 
  group_by(stay_id, itemid) %>% 
  arrange(charttime) %>% 
  slice(1)

# Pivot the data to have one row per ICU stay and columns for each vital measurement
chartevents_tble_final <- chartevents_first %>% 
  pivot_wider(names_from = itemid, values_from = value)
# View the final table
head(chartevents_tble_final)
```
...
# Join with icustays_tble to get one row per ICU stay
chartevents_tble <- chartevents_subset %>%
  left_join(icustays_tble, by = c("stay_id" = "stay_id")) %>%
  select(stay_id, hadm_id, subject_id, 220045, 220179, 220180, 223761, 220210)
...
# Filter for vitals of interest
vitals <- c(220045, 220179, 220180, 223761, 220210)
chartevents_subset2 <- chartevents_tble2 %>%
  filter(itemid %in% vitals) %>%
  select(stay_id, itemid, value, charttime)

# Get the first vital measurement within each ICU stay
chartevents_subset2 <- chartevents_subset2 %>%
  group_by(stay_id, itemid) %>%
  arrange(charttime) %>%
  slice(1) %>%
  ungroup() %>%
